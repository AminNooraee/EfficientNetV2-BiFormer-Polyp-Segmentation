...
class BiLevelRoutingAttention(nn.Module):
    def __init__(
        ...
        if self.kv_downsample_mode == 'ada_avgpool':
            ...
            raise ValueError(
                f'kv_down_sample_mode {self.kv_downsample_mode} is not supported!'
            )
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        k_pix_sel = rearrange(
            k_pix_sel,
            'n p2 k w2 (m c) -> (n p2) m c (k w2)',
            m=self.num_heads,
        )
        v_pix_sel = rearrange(
            v_pix_sel,
            'n p2 k w2 (m c) -> (n p2) m (k w2) c',
            m=self.num_heads,
        )
        q_pix = rearrange(
            q_pix,
            'n p2 w2 (m c) -> (n p2) m w2 c',
            m=self.num_heads,
        )
